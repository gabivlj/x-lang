...
func BenchmarkIntegerArithmetic(t *testing.B) {
	tests := []compilerTestCase{
		...
		{
			input:             "(2 / 1) - 3",
			expectedConstants: []interface{}{2, 1, 3},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
...
func BenchmarkCompilerScopes(t *testing.B) {
	c := New()
	if c.scopeIndex != 0 {
...
func BenchmarkFunctionCalls(t *testing.B) {
	tests := []compilerTestCase{
		{
			input: `fn() { 24 }();`,
			expectedConstants: []interface{}{
...
func BenchmarkLetStatementScopes(t *testing.B) {
	tests := []compilerTestCase{
		{
			input: `
					let num = 55;
...
func BenchmarkCompilerScopesOther(t *testing.B) {
	compiler := New()
